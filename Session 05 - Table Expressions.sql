...
---------------------------------------------------------------------
-- Inline User Defined Functions ( Parameterized Views)
---------------------------------------------------------------------
...
FROM dbo.GetCustOrders(@cid) AS O;

SELECT orderid, custid
FROM dbo.GetCustOrders(1) AS O;

declare @sql as varchar(max)
set @sql = 'SELECT orderid, custid
FROM dbo.GetCustOrders(1) AS O;'
exec (@sql)
...
--- Correlated Table Expression Subquery
...
---Unpivoting
SELECT A.orderid, A.orderdate, d.COLNAME, D.ID
 FROM Sales.Orders AS A
  ...
  ---Applying aggregates horizontally
  SELECT A.orderid,  min(D.colname)
 FROM Sales.Orders AS A
 cross apply ( values ( a.empid, 1)
 , (a.custid, 2)
 , ( a.orderid, 3)
 )d (colname, id)
 group by A.orderid
  ...
  SELECT A.orderid,  A.empid, A.custid
 FROM Sales.Orders AS A
 cross apply (values( case when A.orderid > A.empid then A.orderid else 1 end )
 )d (id)
 where id > 10034
...
FROM Sales.Customers AS C
  CROSS APPLY dbo.TopOrders(C.custid, 10) AS A;
...
SELECT
  ...
  A.orderid, A.empid, A.orderdate, A.requireddate 
FROM Sales.Customers AS C
  inner join (select * from dbo.TopOrders(C.custid, 3) ) as A;
